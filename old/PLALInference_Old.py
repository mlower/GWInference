...
def logL(x, data, PSD, fmin, fmax, deltaF):
    ...
    if m1 >= m1_min and m1 <= m1_max and m2 <= m2_max and m2 >= m2_min and e_min <= ecc_max and e_min >= ecc_min and dist <= dist_max and dist >= dist_min and iota <= angle_max and iota >= angle_min and RA <= angle_max and RA >= angle_min and DEC <= angle_max and DEC >= angle_min and phiRef <= angle_max and phiRef >= angle_min and m1 > m2 :
        ...
        epoch = 1000000008#1167559934.62
...
#data = data_fileH1[1] + 1j*data_fileH1[2]
#data = data[:int(fmax/deltaF)+1]
#psd_fileH1 = np.column_stack( np.loadtxt("./data/data-dumpH1-PSD.dat") )
#PSD = psd_fileH1[1][:int(fmax/deltaF)+1]

##data_file = np.loadtxt('injections/injection.dat',unpack=True)
##Fsd = 1./(data_file[0,1] - data_file[0,0])
##data,freq = tools.nfft(data_file[1,:],Fsd)
##data = data[:int(fmax/deltaF)+1]
...
##PSD_file = PSD_file[:int(fmax/deltaF)+1,:int(fmax/deltaF)+1]
...
plt.savefig('posteriers/walker_m1.png',dpi=200)

## Making corner plot:

import corner
print('making corner plots...')
truths=[21.,8.,0.,300.,(10.*np.pi/180),(20.9375*np.pi/180.),(45.*np.pi/180.),0.]
...
fig.savefig("posteriers/triangle.png")

np.savetxt('evidence.txt', np.c_[lnZ_pt, dlnZ_pt, BF])
np.savetxt('pos.txt', np.c_[pos])
